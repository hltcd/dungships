...
import { ShoppingCart, Star, Code, ArrowRight, CheckCircle2 } from 'lucide-react';
...
export default async function SourceCodePage() {
  ...
  if (session?.user?.id) {
    ...
    userPurchases.forEach((p: any) => {
      if (p.productId) ownedProductIds.add(p.productId);
      if (p.plan?.bonusProducts) {
        p.plan.bonusProducts.forEach((bp: any) => ownedProductIds.add(bp.id));
      }
    });
  }
          ...
          return (
            <Link href={`/source-code/${product.slug}`} key={product.id} className="group relative bg-white/[0.03] backdrop-blur-xl rounded-3xl border border-white/10 overflow-hidden hover:-translate-y-2 transition-all duration-500 shadow-xl hover:shadow-[0_20px_50px_rgba(59,130,246,0.15)] hover:border-blue-500/30">
              ...
              <div className="aspect-video relative overflow-hidden">
                  ...
                  {isOwned && (
                      <div className="absolute top-4 right-4 z-20 bg-green-500 text-white text-[10px] font-black px-3 py-1 rounded-full shadow-lg uppercase tracking-tighter flex items-center gap-1">
                  ...
                  <div className="flex items-center justify-between border-t border-white/5 pt-4">
                      <div className="flex flex-col">
                          <span className="text-[9px] text-gray-500 font-black uppercase tracking-widest mb-1">
                            {isOwned ? "Trạng thái" : "Giá trọn gói"}
                          </span>
                          <div className="flex items-baseline gap-2">
                               {isOwned ? (
                                   <span className="text-lg font-black text-green-400 italic">ĐÃ SỞ HỮU</span>
                               ) : (
                                   <>
                      ...
                      <button className={`p-2.5 rounded-full transition-all shadow-inner ${isOwned ? "bg-green-600/10 text-green-400" : "bg-blue-600/10 group-hover:bg-blue-600 text-blue-400 group-hover:text-white"}`}>
                          {isOwned ? <CheckCircle2 className="w-4 h-4" /> : <ArrowRight className="w-4 h-4" />}
                      </button>
                  </div>
              </div>
            </Link>
